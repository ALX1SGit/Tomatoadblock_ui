...
Jun 24 09:56:02 unknown daemon.info dnsmasq[2743]: query[A] beacons.gcp.gvt2.com                                                             from 192.168.1.114
Jun 24 09:56:02 unknown daemon.info dnsmasq[2743]: query[A] beacons.gcp.gvt2.com                                                             from 192.168.1.132
...
def home():
    ...
    for device in dhcpd_lease:
        ...
        if (match>=1):
            online+=1
            devices.append(str(device).split(",")[0].replace("'","") + " {" + ip + "}")
    print(online)
    print(devices)
    ...
    for line in output:
        ...
        try:
            ...
            flash(toprint)
    return render_template("main.html", lines = lines, block = block, delta=delta, online=online, devices=devices, resolutions=resolutions)
